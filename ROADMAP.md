...
| Version | What | Proof |
|---|---|---|
| v4.0 | **The ecosystem release** ([SPEC-AMENDMENT-3.md](SPEC-AMENDMENT-3.md)): unmodified `wasi:http/proxy@0.2.x` components — Spin apps, componentize-js/JCO output — deploy on the same `/fn` routes as keel-WIT functions. World detection from the compiled export surface at request time (bind stays existence-only); same fuel/mem/time quotas, admission, global permit, and ledger; `wasi:cli` stdout → the fn_logs pipeline; **`wasi:keyvalue`** backed by the Amendment-2 fn_kv store (same caps, same per-ref identity — a keel guest and a wasi guest bound to one ref share data); outbound HTTP is a per-ref **operator grant** (`allow_outbound`, default = clean in-band denial; `keel_fn_outbound_total`); responses capped at 10 MiB (guest_error beyond). Host surface = wasmtime-wasi(-http) 43 **sync** bindings on the one existing engine — the single deliberate wasi dependency family, taken in the spec | `accept_ecosystem.sh` |
| v3.5 | **Functions grow up** ([SPEC-AMENDMENT-2.md](SPEC-AMENDMENT-2.md), WIT 0.7.0 → 0.8.0 — a guest rebuild event, stated loudly): `platform-api` gains `config-get` (per-ref operator config — the API-key unlock; values never echoed by any endpoint) and `kv-get`/`kv-set`/`kv-delete` (durable per-**ref** state: bind one module at two prefixes and they have two stores; `kv-set` returns only after the row commits — kill -9 proven; caps key ≤ 256 B, value ≤ 64 KiB, 1024 keys / 8 MiB per ref, checked atomically; per-call atomicity only — CAS deliberately not built, workflows are the atomic path). Control plane: `POST/GET/DELETE /api/config` (names-only), `GET/DELETE /api/kv` (keys-only, wipe). Solver binaries unaffected (import-free world) | `accept_functions2.sh` + the whole suite rebuilt green under 0.8.0 |
